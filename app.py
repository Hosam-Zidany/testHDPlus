...
class StudentDataFetcher:
    ...
    def fetch_student_data(self, student_id):
        ...
        try:
            ...
            # Extract year from last panel-title
            panel_titles = soup.find_all(class_="panel-title")
            year = panel_titles[-1].text.strip() if panel_titles else "غير معروف"
            ...
            logger.error("Timeout occurred while fetching data")
            return {"error": "حدث خطأ في جلب البيانات من الموقع بسبب خطأ في الخادم"}
...
• حاول مرة أخرى لاحقاً"""
    ...
    async def inline_query_handler(
        ...
        if not query:
            # Show placeholder when no query is provided
            results = [
                InlineQueryResultArticle(
                    id="placeholder",
                    title="🔍 أدخل الرقم الجامعي",
                    description="اكتب الرقم الجامعي للبحث عن علامات الطالب",
                    input_message_content=InputTextMessageContent(
                        message_text="🔍 أدخل الرقم الجامعي للبحث عن علامات الطالب",
                        parse_mode="Markdown",
                    ),
                )
            ]
        ...
        # Validate if query contains only digits
        if not query.isdigit():
            results = [
                InlineQueryResultArticle(
                    id="invalid",
                    title="❌ رقم غير صالح",
                    description="الرقم الجامعي يجب أن يكون مكوناً من أرقام فقط",
                    input_message_content=InputTextMessageContent(
                        message_text="❌ الرقم الجامعي يجب أن يكون مكوناً من أرقام فقط\n\n📝 *مثال:* `202112345`",
                        parse_mode="Markdown",
                    ),
                )
            ]
        ...
        # Fetch student data
        student_data = self.fetcher.fetch_student_data(query)

        if student_data is None or (
            isinstance(student_data, dict) and "error" in student_data
        ):
            error_msg = (
                student_data.get(
                    "error", f"لم يتم العثور عن طالب بالرقم الجامعي: {query}"
                )
                if isinstance(student_data, dict)
                else f"لم يتم العثور عن طالب بالرقم الجامعي: {query}"
            )
            results = [
                InlineQueryResultArticle(
                    id="not_found",
                    title="❌ خطأ في جلب البيانات",
                    description=f"خطأ في جلب بيانات الطالب {query}",
                    input_message_content=InputTextMessageContent(
        ...
        # Format the student data for inline result
        formatted_message = self.format_student_message(student_data)

        results = [
            InlineQueryResultArticle(
                id=f"student_{query}",
                title=f"📊 علامات {student_data['name']}",
                description=f"عرض علامات الطالب {student_data['name']} - الرقم: {query}",
                input_message_content=InputTextMessageContent(
    ...
    def format_student_message(self, student_data):
        ...
        # Add year header
        lines.append(f"📅 {student_data['year']}")
        lines.append("")

        # Display subjects with year
        ...
        # Summary
        lines.append("")
        lines.append(f"📊 المعدل: *{avg_mark:.1f}*")
    ...
    async def handle_student_id(
        ...
        if student_data is None or (
            ...
            error_msg = (
                student_data.get("error", "لم يتم العثور على الطالب")
                if isinstance(student_data, dict)
                else "لم يتم العثور على الطالب"
            )
            await update.message.reply_text(
                f"❌ {error_msg}\n\n"
                "🔍 *تأكد من:*\n"
